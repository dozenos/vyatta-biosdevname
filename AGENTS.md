...
DozenOS fork of `biosdevname` — a Dell-originated utility that maps Linux kernel network-device names (e.g. `eth0`) to BIOS/chassis-supplied names (e.g. `Gb1`). Ships as the `vyatta-biosdevname` Debian package and is invoked by udev rules during interface naming.
...
Listed in an internal repository. Pulled into the ISO via `dozenos/dozenos-build`. Provides predictable interface naming used throughout `dozenos-1x`'s ifconfig logic.
...
- Any change here can affect interface naming on every DozenOS install; coordinate with `dozenos-1x` if you change rule names.
